...
C      if (abs(ibeam).gt.1) then
C        write (*,*) 'GETSTR: ibeam = ',ibeam,' not implemented'
C        call error
C      endif                     !abs(ibeam)
...
C ! TAKING U1,D1,U2,D2 --> UB1,DB1,UB2,DB2.       
      ...
      QQBZ = (UDB+UBD)*XUDB + (DBU+DUB)*XDBU + (UUB1+UBU1)*XUUB1      
     1     + (DDB1+DBD1)*XDDB1 + (UUB+UBU)*XUUB + (DDB+DBD)*XDDB      
      RETURN        
      END 
...
C     !!!! CAREFUL -- SCREWUP IF WT EVER GETS SWAPPED.      
     ...
     1  (11./6.*NC-2./3.*TR)*(-WA+WQ) + (67./18.*NC-10./9.*TR)         
