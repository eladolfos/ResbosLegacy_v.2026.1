...
!      XDD  = IFLAG*GVD**2*FV5678 + GAD**2*FA5678 + GGD*(FF5656+FF7878)      
      XUD = GGU*FF5656 
     1        + IFLAG*GVU*GVD*FV5678 *(GGU/(GGU+GGD))
     &        + GAU*GAD*FA5678 *(GGU/(GGU+GGD))
      XDU = GGU*FF7878 
     2        + IFLAG*GVU*GVD*FV5678 *(GGU/(GGU+GGD))
     &        + GAU*GAD*FA5678 *(GGU/(GGU+GGD))
      XUU  = IFLAG*GVU**2*FV5678 + GAU**2*FA5678 + GGU*(FF5656+FF7878)      
      XDD  = 0!IFLAG*GVD**2*FV5678 + GAD**2*FA5678 + GGD*(FF5656+FF7878)      
...
!     1          + GAD*(NU*GAU+ND*GAD)*FA1234        
      GGSUM = NU*GGU  
      XUUB1 = GGU*(FF1256+FF1278+FF3456+FF3478) + GGSUM*FF3434
     1          + GAU*(NU*GAU+ND*GAD *(GGU/(GGU+GGD))   )*FA1234        
      XDDB1 =  GGSUM*FF3434
     1          + GAD*(NU*GAU *(GGU/(GGU+GGD)) )*FA1234        
...
!      QQBZX =  (GAU*(UUB1+UBU1)+GAD*(DDB1+DBD1)) * (NU*GAU+ND*GAD) * ZX
      QQBZX =  (GAU*(UUB1+UBU1)) * (NU*GAU) * ZX
     &  + (GAU*(UUB1+UBU1)) * (ND*GAD) * ZX *(GGU/(GGU+GGD))
     &  + (GAD*(DDB1+DBD1)) * (NU*GAU) * ZX *(GGU/(GGU+GGD))
!	QQBZX=0
      RETURN
      END
...
!     1     + (GAU*(GU+GUB)+GAD*(GD+GDB)) * (GAU*NU+GAD*ND) * ZXGQ
      QGZX = (GAU*(UG+UBG)) * (GAU*NU) * ZXQG
     1     + (GAU*(GU+GUB)) * (GAU*NU) * ZXGQ
     &     + (GAU*(UG+UBG)) * (GAD*ND) * ZXQG *(GGU/(GGU+GGD))
     &     + (GAU*(GU+GUB)) * (GAD*ND) * ZXGQ *(GGU/(GGU+GGD))
     &     + (GAD*(DG+DBG)) * (GAU*NU) * ZXQG *(GGU/(GGU+GGD))
     &     + (GAD*(GD+GDB)) * (GAU*NU) * ZXGQ *(GGU/(GGU+GGD))
!	QGZX=0
      RETURN
      END
...
CZL
      ...
      XDD1 = 0!(1./2.)*GGD*(HH1256+HH1278+HH3456+HH3478)      
...
C **  THE DELTA(S2) PIECES OF SECOND-ORDER CONTRIBUTIONS.   
      ...
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)         
        ...
        SAVE SWFLAG 
      COMMON /STU/ SS,TT,UU,QQ,QM,QMM,qmu
      COMMON /XINDS2/ SPT,SPU,TPU,WS,WT,WU,WQ,WA,WMu
      TPU = T+U     
      SPU = S+U     
      SPT = S+T     
      WS = LOG(S/QQ)
