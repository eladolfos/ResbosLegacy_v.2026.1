...
CCPY          PARTFN = GGU*(UUB1+UBU1) !+ GGD*(DDB1+DBD1)

CCPY
          IF(LEPASY.EQ.1) THEN
           PARTFN = GGU*(UUB1-UBU1) + GGD*(DDB1-DBD1)
cdump  
C       print *,' QQB1: PARTFN  =0 = ',PARTFN 
C        PARTFN=0.d0
...
CCPY        QG = (GGU*(UG+UBG) ) * XQG  
CCPY     1    + (GGU*(GU+GUB) ) * XGQ  
CCPY
        IF(LEPASY.EQ.1) THEN
          QG = (GGU*(UG-UBG) + GGD*(DG-DBG)) * XQG  
     1    + (GGU*(-GU+GUB) + GGD*(-GD+GDB)) * XGQ  
...
CZL
     ...
     1        + IFLAG*GVUXGVD*FV5678 + GAUXGAD*FA5678 
      XDU = GGU*FF7878 + GGD*FF5656
     2        + IFLAG*GVUXGVD*FV5678 + GAUXGAD*FA5678 
      XUU  = IFLAG*GVUXGVU*FV5678 + GAUXGAU*FA5678 + GGU*(FF5656+FF7878)      
      XDD  = IFLAG*GVDXGVD*FV5678 + GADXGAD*FA5678 + GGD*(FF5656+FF7878)      

CCPY      XUD = GGU*FF5656 
C     1        + IFLAG*GVUXGVD*FV5678 
C     &        + GAUXGAD*FA5678 
C      XDU = GGU*FF7878 
C     2        + IFLAG*GVUXGVD*FV5678
C     &        + GAUXGAD*FA5678 
C      XUU  = IFLAG*GVUXGVU*FV5678 + GAUXGAU*FA5678 + GGU*(FF5656+FF7878)      
C      XDD  = 0!IFLAG*GVDXGVD*FV5678 + GADXGAD*FA5678 + GGD*(FF5656+FF7878)      
...
CZL
     ...
     1          + (GAUXGAU*NU+GAUXGAD*ND)*FA1234        
      XDDB1 = GGD*(FF1256+FF1278+FF3456+FF3478) + GGSUM*FF3434
     1          + (GAUXGAD*NU+GADXGAD*ND)*FA1234        

CCPY      GGSUM = NU*GGU  
C      XUUB1 = GGU*(FF1256+FF1278+FF3456+FF3478) + GGSUM*FF3434
C     1          + GAU*(NU*GAU+ND*GAD *(GGU/(GGU+GGD))   )*FA1234        
C      XDDB1 =  GGSUM*FF3434
C     1          + GAD*(NU*GAU *(GGU/(GGU+GGD)) )*FA1234        
...
!     QQBZX =  (GAU*(UUB1+UBU1)+GAD*(DDB1+DBD1)) * (NU*GAU+ND*GAD) * ZX
      QQBZX =  (GAUXGAU*(UUB1+UBU1)+GAUXGAD*(DDB1+DBD1)) * NU * ZX
     & + (GAUXGAD*(UUB1+UBU1)+GADXGAD*(DDB1+DBD1)) * ND * ZX

CCPY      QQBZX =  (GAU*(UUB1+UBU1)) * (NU*GAU) * ZX
C     &  + (GAU*(UUB1+UBU1)) * (ND*GAD) * ZX *(GGU/(GGU+GGD))
C     &  + (GAD*(DDB1+DBD1)) * (NU*GAU) * ZX *(GGU/(GGU+GGD))
C!	QQBZX=0
CCPY
       IF(LEPASY.EQ.1) THEN
        QQBZX =  (GAUXGAU*(UUB1-UBU1)+GAUXGAD*(DDB1-DBD1)) * NU * ZX
...
!     1     + (GAU*(GU+GUB)+GAD*(GD+GDB)) * (GAU*NU+GAD*ND) * ZXGQ
      ...
      QGZX = (GAUXGAU*(UG+UBG)+GAUXGAD*(DG+DBG)) * NU * ZXQG
     &     + (GAUXGAD*(UG+UBG)+GADXGAD*(DG+DBG)) * ND * ZXQG
     &     + (GAUXGAU*(GU+GUB)+GAUXGAD*(GD+GDB)) * NU * ZXGQ
     &     + (GAUXGAD*(GU+GUB)+GADXGAD*(GD+GDB)) * ND * ZXGQ

CCPY      QGZX = (GAU*(UG+UBG)) * (GAU*NU) * ZXQG
C     1     + (GAU*(GU+GUB)) * (GAU*NU) * ZXGQ
C     &     + (GAU*(UG+UBG)) * (GAD*ND) * ZXQG *(GGU/(GGU+GGD))
C     &     + (GAU*(GU+GUB)) * (GAD*ND) * ZXGQ *(GGU/(GGU+GGD))
C     &     + (GAD*(DG+DBG)) * (GAU*NU) * ZXQG *(GGU/(GGU+GGD))
C     &     + (GAD*(GD+GDB)) * (GAU*NU) * ZXGQ *(GGU/(GGU+GGD))
C!	QGZX=0
CCPY
       IF(LEPASY.EQ.1) THEN
        QGZX = (GAUXGAU*(UG-UBG)+GAUXGAD*(DG-DBG)) * NU * ZXQG
...
CZL
      XUU1 = (1./2.)*GGU*(HH1256+HH1278+HH3456+HH3478)      
