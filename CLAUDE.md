...
`resbos_campaign.py` + `campaign_example.ini` (+ `shrds_scripts/`) prepare and chain a whole run from one INI: `python3 resbos_campaign.py campaign.ini [--submit] [--reuse]`. The INI names a `source` folder (holding `get_yk_new/ legacy/ resbos/ w_asym/ w_pert/`; `templates/7TeV_WpWm/` is the reference one) and a new `dest`. It copies only what a run needs (executables, the `inp/*.inp` grids, `make_dummy_rai.py`; ~9 MB), rewrites the `.in` files, and writes `dest/submit_all.sh`, which chains SLURM jobs with `--dependency=afterok` by parsing the `Submitted merge job N` line printed by the shard submit scripts.
...
- **Executables** come from `<source>/<folder>/<exe>` unless `[executables]` says otherwise (`dir = <dir>` with the `<dir>/<folder>/<exe>` layout, or one key per code: `w_pert`, `w_asym`, `legacy`, `get_yk_new`, `resbos`; a key wins over `dir`). The template binaries are stale snapshots: `setup_resbos_legacy.sb` (plain bash, no python/conda; `sbatch` it or `bash -l` it on the HPCC, from this folder) copies the top-level sources of the five source dirs to `build/<folder>/`, does a from-scratch `make` with module/LHAPDF/HOPPET/ROOT settings taken from env vars (`MODULES`, `LHAPDF`, `HOPPET`, `ROOT_SETUP`; defaults = the HPCC paths of `[environment]`), and writes `bin/<folder>/<exe>` (`OUT=templates/<name>` refreshes a template; `ONLY="legacy resbos"` builds a subset). It overrides Legacy's hardcoded `HOPPETLIBS` with `$HOPPET`. The source repos and their checked-in binaries are never touched. Tested locally only with a fake `gfortran` (no real compiler here).
