...
CCPY  Dec 1, 2020:    al2pi = alpi(qm)/2.0
      al2pi = alpi(qmu)/2.0
...
CBY add the asymetrical part
      function charge(sig, asyi)
      IMPLICIT NONE
...
CCPY Feb 2021
     ...
     3       + cc21*sig(ndb1,nc2) + cc22*sig(nstb1,nc2)
        elseif(asyi.eq.1) then
         udbk = cc11*sig(nu1,ndb2) + cc12*sig(nu1,nstb2)
     1       + cc21*sig(nc1,ndb2) + cc22*sig(nc1,nstb2)
     2       - cc11*sig(ndb1,nu2) - cc12*sig(nstb1,nu2)
     3       - cc21*sig(ndb1,nc2) - cc22*sig(nstb1,nc2)
CJI Feb 2022
C Account for minus sign induced by trick to use same code for W+/-
C See lines: 133-147
         if(jwtype.eq.jwm) then
             udbk = -udbk
         endif
...
CBY add asym part
      function adypt(qt0,y0,mord0,asyi)
      IMPLICIT NONE
...
ccpy
      ...
      if(abs(xa).ge.1.0 .or.abs(xb).ge.1.0) then    
        ...
        adypt = gevxpb * pi/3./ss/qt**2 * charge(asig,asyi)
      endif
